...
def gerar_url_data(semanas_atras=0):
    hoje = datetime.now()
    # Calcula o domingo de semanas_atras atrás
    ultimo_domingo = hoje - timedelta(days=hoje.weekday() + 8 + 7*semanas_atras)
...
def baixar_arquivo():
    for semanas_atras in range(0, 3):  # Tenta as 3 últimas semanas
        url, data_inicio, data_fim = gerar_url_data(semanas_atras)
